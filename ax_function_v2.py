...
def ax_2(date_naissance_X, sexe_X, date_naissance_Y, sexe_Y, date_liquidation, date_calcul, age_depart, frac, methode_age_atteint, tx_reversion, prorata_deces, rattrapage_rente, terme, tx_contre_assurance, tx_frais_rente, option_liquidation, type_calcul="Constitution") :
    """
    Retourne l'annuité X2 comme calculé par AXA
    """
    date_calcul_outil = datetime(date_calcul.year, date_calcul.month, 1)

    if option_liquidation in ["5 annuites garanties", 
                              "10 annuites garanties", 
                              "15 annuites garanties", 
                              "10 annuites garanties avec reversion", 
                              "15 annuites garanties avec reversion"] :
        
        nombre_annuites_garanties = int(re.search(r'\d+', option_liquidation).group())
        pourcentage_augmentation_rente_70_75 = 0
        nombre_annees_arrerage_majores_deces = 0

    elif option_liquidation == "Paliers 70 et 75 ans" :

        nombre_annuites_garanties = 0
        pourcentage_augmentation_rente_70_75 = 0.2
        nombre_annees_arrerage_majores_deces = 0

    elif option_liquidation in ["Arrérages majorés au décès 100%",
                                "Arrérages majorés au décès 200%",
                                "Arrérages majorés au décès 300%"] :
    
        nombre_annuites_garanties = 0
        pourcentage_augmentation_rente_70_75 = 0
        nombre_annees_arrerage_majores_deces = int(re.search(r'[123]', option_liquidation).group(0))
    ...
    date_calcul_modifie = max(date_liquidation, date_calcul_outil)

    seconde_periode = fin_frac(date_liquidation, frac)

    age_X_liquidation = age_precis_2(date_naissance_X, date_liquidation)
    age_fin_annuite_garantie = age_X_liquidation + nombre_annuites_garanties
    ...
    while min(age_X_temp, age_Y_temp) < 121 :
        ...
        if 70 <= age_X_temp < 75 :
            flux_base_rente[-1] = flux_base_rente[-1] * (1 + pourcentage_augmentation_rente_70_75) ** 1
        elif age_X_temp >= 75 :
            flux_base_rente[-1] = flux_base_rente[-1] * (1 + pourcentage_augmentation_rente_70_75) ** 2

        if nombre_annuites_garanties > 0 and age_fin_annuite_garantie > age_X_temp - flux_base_rente[-1] :
            Px_X.append(1)
        else :
        ...
        flux_arrerage_maj_deces.append(Qx_X[-1] * flux_base_rente[-1] * frac * nombre_annees_arrerage_majores_deces)
    ...
    ax_arrerage_majo_deces_avec_contreassurance = sum(flux_arrerage_maj_deces) * (1 + tx_frais_rente)

    ax_avec_contreassurance = ax_X_avec_contreassurance + ax_Y_avec_contreassurance + ax_prorata_deces_avec_contreassurance + ax_arrerage_majo_deces_avec_contreassurance

    age_X_calcul = age_precis_2(date_naissance_X, date_calcul_outil)
    if tx_reversion == 0 :
    ...
    ax_arrerage_majo_deces_sans_contreassurance = sum(flux_arrerage_maj_deces) * (1 + tx_frais_rente)

    ax_sans_contreassurance = ax_X_sans_contreassurance + ax_Y_sans_contreassurance + ax_prorata_deces_sans_contreassurance + ax_arrerage_majo_deces_sans_contreassurance

    ax_axa = ax_sans_contreassurance + tx_contre_assurance * (ax_avec_contreassurance - ax_sans_contreassurance)

    return(ax_axa)

print(ax_2(datetime(1972,10,4), 1, datetime(1950, 1, 1), 0, datetime(2037, 1, 1), datetime(2023, 6, 30), 64, 4, 40, 0, "COMP", False, "ECHU", 1, 0.01, "", "Constitution"))