...
def formattage(df) :
    ...
    df["date_naissance_Y"] = df["date_naissance_Y"].apply(lambda row : date_naissance_conjoint_fictif(row["Raison sociale"], row["date_naissance_X"]))
    df["sexe_Y"] = df["sexe_Y"].apply(lambda row : sexe_conjoint_fictif(row["sexe_X"]))
